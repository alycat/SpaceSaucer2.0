	...
	)
{
	...
	{
		GSOutput element;
		float index = quadIndex[i];
		element.position = mul(v[index], worldViewProj);
		element.normal = normalize(mul(input[0].normal, (float3x3)world));
		element.uv = quadUVs[index];
